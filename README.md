...
1. **Create the specialists.** Run `python create_specialists.py`. This creates five sub-agents (Pricing, Legal, Technical Fit, Competitive, and Comparison/Benchmarking) and saves their IDs to `.specialist_ids.json`.

2. **Upload the skills.** Run `python upload_skills.py`. This packages the custom skills in `skills/` and uploads them via the Skills API. Each specialist gets the skill that matches its domain.

3. **Create the coordinator.** Run `python create_coordinator.py`. This creates the coordinator agent with `multiagent: coordinator` config, listing the specialists in its callable roster.

4. **Create the session environment.** Run `python setup_environment.py`. This provisions the cloud environment used by the Deal Desk session and saves its ID to `.environment_id`.

5. **Run the deal.** Run `python run_deal_desk.py`. This:
   ...
   - Saves the final docx to `outputs/proposal-response.docx`
    ...
    For the live host view, run `python run_deal_desk_with_commentary.py` instead. It watches the same event stream, prints human commentary beside the raw events, and saves it to `outputs/swarm-commentary.txt` alongside the downloaded docx.
...
- **Critic sub-agent** — add a sixth agent that reviews the coordinator's draft before it's finalised
...
Two-monitor setup:
- **Monitor 1:** the events stream from the coordinator session, scrolling. You'll see `session.thread_created` × 5, parallel `running`, then `agent.thread_message_received` flowing back, with live commentary in the terminal. The visible parallelism IS the demo.
...
├── run_deal_desk_with_commentary.py (runs the swarm with live commentary)
...
│   ├── deal-history-comparison/SKILL.md
│   └── swarm-commentary/SKILL.md
└── synthetic-data/
